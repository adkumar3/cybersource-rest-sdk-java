...
public class ShipToTest {
    ...
    public void setup() throws URISyntaxException, IOException {
        ...
        File file = new File(Thread.currentThread().getContextClassLoader().getResource("ShipTo.json").toURI());
    ...
    public void testDeserialize() throws ApiException, IllegalAccessException, InvocationTargetException {
        TypeRef typeRef = new TypeRef<com.cybersource.payments.model.ShipTo>() {};
        JSON json = new JSON();
        ShipTo instanceOfShipTo = json.deserialize(serialized, typeRef);
        Method[] methods = ShipTo.class.getMethods();

        for(Method method : methods){
            if (!Modifier.isPublic(method.getModifiers())) {
                continue;
            }
            if (method.getDeclaringClass() == ShipTo.class && method.getName().startsWith("get")) {
                assertTrue(method.invoke(instanceOfShipTo) != null);
            }
        }
	}
}
